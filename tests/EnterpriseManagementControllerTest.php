...
class EnterpriseManagementControllerTest extends TestCase {
    ...
    public function testUpdateCompanySuccess() {
        ...
        $this->modelMock
            ->method('updateCompanyAccount')
            ->willReturn("updated");

        $this->expectOutputRegex('/.*/');

        $this->controller->updateCompany();
    }
    public function testDeleteCompanyCallsModel() {
    ...
    public function testRateCompanyCallsModel() {
        ...
        $_POST["note"] = 5;
        $_POST["commentaire"] = "Top";

        $this->modelMock
            ->expects($this->once())
            ->method('saveManagementEvaluation')
            ->with(1, $_SESSION["admin"], 5, "Top")
            ->willReturn("rated");

        $this->expectOutputRegex('/.*/');

        $this->controller->rateCompany();
    }
}
...
$genresAutorises = ['ms', 'mr', 'other'];
$rolesAutorises = ['etudiant', 'professeur'];
...
try {
    ...
    $stmtCompte = $pdo->prepare("
        ...
        VALUES (:email, :mot_de_passe, :role)
    ");
    $stmtCompte->execute([
        ':email' => $email,
    ...
    if ($role === 'etudiant') {
        ...
        $stmtEtudiant = $pdo->prepare("
            INSERT INTO etudiants (compte_id, nom, prenom, genre, cv_nom_original, cv_nom_stocke, cv_chemin, cv_taille)
            VALUES (:compte_id, :nom, :prenom, :genre, :cv_nom_original, :cv_nom_stocke, :cv_chemin, :cv_taille)
        ");
    ...
    if ($role === 'professeur') {
        $stmtProfesseur = $pdo->prepare("
            INSERT INTO professeurs (compte_id, nom, prenom, genre, specialite)
            VALUES (:compte_id, :nom, :prenom, :genre, :specialite)
        ");
